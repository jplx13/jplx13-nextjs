...
export default function ChatApp() {
  ...
  const [conversations] = useState<Conversation[]>([
    {
      ...
      active: true,
      lastMessage: "Exploring modern design principles and their applications in contemporary buildings.",
    ...
    },
    {
      ...
      active: false,
      lastMessage: "Discussing software architecture patterns and best practices.",
    ...
    },
    {
      ...
      active: false,
      lastMessage: "Understanding the core principles of modern development.",
      timestamp: "1 day ago",
    },
    {
      id: 4,
      ...
      active: false,
      lastMessage: "Analyzing user interface trends and user experience methodologies.",
  ...
  }, [agentState.selected, selectedFile, selectAgent, removeFile])
  ...
  const retryApiCall = async (formData: FormData, attempt = 1): Promise<APIResponse> => {
    const maxRetries = 3

    try {
      console.log(`🚀 JPLx13-HE API Call - Attempt ${attempt}`, {
      ...
      console.log(`✅ JPLx13-HE API Success - Attempt ${attempt}`, {
        agent: data.response?.agent,
        model: data.response?.model,
        hasDownload: !!data.document?.downloadUrl,
        responseLength: data.response?.content?.length || 0,
        timestamp: new Date().toISOString(),
      })

      return data
    } catch (error) {
      console.error(`❌ JPLx13-HE API Error - Attempt ${attempt}:`, {
      ...
      if (attempt < maxRetries) {
        ...
        return retryApiCall(formData, attempt + 1)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (input.trim()) formData.append("chatInput", input)
    if (selectedFile) {
    ...
    if (agentState.selected !== "auto") {
      formData.append("selectedAgent", agentState.selected)
    ...
    try {
      setProgress(50)
      const data = await retryApiCall(formData)
      setProgress(100)
      ...
      setError(`Connection failed after ${retryCount + 1} attempts. Please try again.`)
            ...
            <ScrollArea className="flex-1 px-4 scrollbar-thin">
              <div className="space-y-2">
                {conversations.map((conv) => (
                  <div
                    key={conv.id}
                    className="conversation-item cursor-pointer transition-all duration-200"
                    style={{
                      padding: "12px",
                      marginBottom: "8px",
                      color: "#e0e0e0",
                  ...
                  >
                    <div className="conversation-title font-medium text-sm mb-1" style={{ fontSize: "15px" }}>
                      {conv.title}
                    </div>
              ...
              <div className="space-y-4">
                {messages.length === 0 && (
                  <div className="text-center py-8 sm:py-12">
                    ...
                    >
                      Your AI assistant with specialized agents for different tasks. Select an agent below or let Auto
                      mode choose the best one for your query.
                    </p>
            ...
            <div className="p-4 sm:p-6 flex-shrink-0">
              ...
              <div className="mb-4">
                ...
                <div className="text-xs mt-2 text-center" style={{ color: "#a0a0a0" }}>
                  Press Tab to cycle through agents • Escape to clear file
                </div>
              </div>
